...
select f.id, f.名称 班级, f.年级, a.名称 教材, b.名称 出版社, a.ISBN, a.价格
from 教材 a
         inner join 出版社 b on a.出版社ID = b.id
         inner join 课程教材表 c on c.教材ID = a.id
         inner join 课程 d on c.课程ID = d.id
         inner join 班级课程表 e on d.id = e.课程ID
         inner join 班级 f on e.班级ID = f.id;

-- 学生需预购教材清单
create or replace view 学生需预购教材清单 as
select a.学号, a.姓名, b.教材, b.ISBN, b.价格, b.出版社
from 学生 a
...
select b.id, b.名称, a.学年, a.学期, COUNT(教材ID) as 总数量
from 教材预定表 a
         left join 教材 b on a.教材ID = b.id
group by a.学年, a.学期, 教材ID, b.名称;

create or replace view 需订购教材清单 as
select a.id,
       a.名称,
       a.ISBN,
       b.总数量 - a.库存数量            as 需订数量,
       (b.总数量 - a.库存数量) * a.价格 as 价格,
       c.名称                           as 出版社名称,
       c.联系电话,
       c.地址,
       b.学年,
       b.学期
from 教材 a
         right join 教材预定清单 b on a.id = b.id
         left join 出版社 c on a.出版社ID = c.id
where a.库存数量 < b.总数量;
